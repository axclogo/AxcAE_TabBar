...
@interface ItemCustomAnimationTabBarVC ()<AxcAE_TabBarDelegate>
...
- (void)addChildViewControllers{
     ...
     */
    NSArray *customAnimationArray = @[@(AxcAE_TabBarInteractionEffectStyleSpring), // 放大放小弹簧效果
                                  @(AxcAE_TabBarInteractionEffectStyleShake),      // 摇动动画效果
                                  @(AxcAE_TabBarInteractionEffectStyleAlpha),      // 透明动画效果
                                  @(AxcAE_TabBarInteractionEffectStyleCustom),     // 自定义动画效果 4
                                  @(AxcAE_TabBarInteractionEffectStyleCustom)];    // 自定义动画效果 5
    ...
    [VCArray enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (idx) {
            ...
            case 3:
                model.itemSize = CGSizeMake(70, 40);
                model.itemBadgeStyle = AxcAE_TabBarItemBadgeStyleTopLeft;
                model.badge = [NSString stringWithFormat:@"%d",arc4random()%100 + 50];
                break;
            case 4:
                model.itemLayoutStyle = AxcAE_TabBarItemLayoutStyleLeftPictureRightTitle;
                break;
            default: break;
        }
        // DEMO 设置动画模式
        ...
        model.tag = 100 + idx;
        // 自定义动画回调Block
        model.customInteractionEffectBlock = ^(AxcAE_TabBarItem *item) {
